...
\section{アルゴリズムの概要}
\rout{配置優先度を用いた試薬合成におけるフラッシング回数の削減手法は，配置優先度\bout{と呼ばれる}指標を用いて，試薬合成中に行われるフラッシング回数を削減する既存の手法である~\cite{10089903}．}
...
配置優先度を用いた試薬合成におけるミキサーの配置の決定順序の並び替えアルゴリズムの狙いは，ミキサーのオーバーラップを発生させる可能性の高い部分木内の混合ノードほど，PMD上に空きセルが多い，小さいタイムステップで優先的に配置方法を\rout{決定することである．}
