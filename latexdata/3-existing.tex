...
\section{アルゴリズムの概要}
\rout{配置優先度を用いた試薬合成におけるフラッシング回数の削減手法は，配置優先度と\bout{と呼ばれる}指標を用いて，試薬合成中に行われるフラッシングの回数を削減する既存の手法である~\cite{10089903}．}
\bout{以降，この手法を平井らの手法と呼ぶ．}
本節では，平井らの手法\rout{の}処理の流れを説明する．

まず，平井らの手法での手法全体の入出力データについて説明を行う．
平井らの手法の入力は，図~\ref{fig:xntm}（a）の混合木である．
平井らの手法の出力は，図~\ref{fig:xntm}（b）から（e）の2x2ミキサーと2x3ミキサーを用いたPMD上\rout{での試薬合成における}液滴の混合手順である．
図~\ref{fig:xntm}（b）から（e）における値Tはミキサーの混合が行われたタイムステップ，値Fはそのタイムステップに至るまでに行ったフラッシング回数を表している．
\bout{平井らの手法は，図~\ref{fig:xntm}（a）の混合木の試薬合成中に，フラッシングを1回行っている．}
...
%次に，平井らの手法の1つ目の処理である，配置優先度を用いた試薬合成におけるミキサーの配置の決定順序の並び替えアルゴリズムを説明する．
...
%まず，平井らの手法の2つ目の処理である，PMD上での2x3ミキサーを用いた液滴の混合手順生成のアルゴリズムについて説明する．
